...
class StoreOprDailyCityPerformanceRequest extends FormRequest
{
    /**
    ...
    {
        $data = OprDailyCityPerformance::where('inbound_date', $this->only('inbound_date'))->where('zone', $this->only('zone'))->where('hub', $this->only('hub'))->first();
        if ($data) {
